...
namespace cxxtlib
{
	namespace format
	{
		...
		public:
			explicit Writer(details::uint32 pCapacity = 4) CXXTLIB_FORMAT_NOEXCEPT
		...
		static CXXTLIB_FORMAT_CONSTEXPR CXXTLIB_FORMAT_INLINE Char* format(const Char* const pPattern, Arguments&&... pArguments) CXXTLIB_FORMAT_NOEXCEPT
		{
			...
			Writer<Char> writer = Writer<Char>(patternLength + argumentsLength);
			// Passing reader, writer and arguments to the handle function and returning formatted cstring
			formatHandle<Char, Reader<Char>, Writer<Char>, Arguments...>(reader, writer, details::forward<Arguments>(pArguments)...);
			return writer.get();
		}
		...
		static CXXTLIB_FORMAT_CONSTEXPR CXXTLIB_FORMAT_INLINE Char* format(const Char* const pPattern, Arguments&&... pArguments) CXXTLIB_FORMAT_NOEXCEPT
		{
			...
			Writer<Char> writer = Writer<Char>(tSize);
			// Passing reader, writer and arguments to the handle function and returning formatted cstring
			formatHandle<Char, Reader<Char>, Writer<Char>, Arguments...>(reader, writer, details::forward<Arguments>(pArguments)...);
			return writer.get();
		}
